...
public class CryptogramController {
 ...
 public ArrayList<String> show;
	 ...
	 public ArrayList showWord ()  {
		 int x;
		 int y;
		 String word2=model.getEncryptedString();
			...
			for(int i=0;i<13;i++) {
				...
				for(int j=0;j<word2.length();j++) {
					if((int)word2.charAt(j)==word1) {
						y++;
					}
					if((int)word2.charAt(j)==word) {
						x++;
					}
				}
				if(x>=10) {
				show.add((char)word+"  "+x+"         "+(char)word1+"  "+y);}else {
					show.add((char)word+"  "+x+"        "+(char)word1+"  "+y);
				}
				word=word+1;
				word1=word1+1;
			}
	 ...
	 */
	public String getEncryptedQuote1() { /* for the view to display */
		String test1="";
		String test2="";
		int wordLength=0;
				...
				if(!(word.charAt(i)>='A' && word.charAt(i)<='Z')) {
					...
					if(temp>=30) {
						...
						if(test1.charAt(test1.length()-1)==' ') {
							 StringBuilder strBuilder = new StringBuilder(test1);
							 strBuilder.setCharAt(test1.length()-1,'$');
							 out1+=strBuilder.toString();
						 }else {
							out1+=test1+"$";}
